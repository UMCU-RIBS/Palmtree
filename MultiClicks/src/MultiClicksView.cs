...
namespace MultiClicksTask {
    ...
    /// <summary>
    /// The <c>MultiClicksView</c> class.
    /// 
    /// ...
    /// </summary>
    public class MultiClicksView : OpenTKView, IView {
    //public class MultiClicksView : SharpGLView, IView {

        public const int noBlock = -1;

        private static Logger logger = LogManager.GetLogger("MultiClicksView");         // the logger object for the view
		...
		private List<MultiClicksBlock> blocks = new List<MultiClicksBlock>(0);	        // block objects
        ...
        public MultiClicksView() : base(60, 0, 0, 640, 480, true) {
            
        }

        public MultiClicksView(int updateFrequency, int x, int y, int width, int height, bool border) : base(updateFrequency, x, y, width, height, border) {
        ...
        protected override void render() {
	        ...
	        if (showBlocks) {
		        ...
		        for (int i = 0; i < blocks.Count; ++i) {
                    ...
                    if (blocks[i].texture == 0) {
                        // no texture

                        // set the blockcolor
                        glColor3(blocks[i].colorR, blocks[i].colorG, blocks[i].colorB);
